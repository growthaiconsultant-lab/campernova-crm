...
### Cómo activarlos (Windows + Codex Desktop)

**Importante**: en Codex Desktop, los `mcpServers` definidos en `.Codex/settings.local.json` no se cargan de forma fiable. Hay que registrarlos con la CLI `Codex mcp add-json`. Cada dev hace esto una vez en su máquina.

1. Instala la CLI si no la tienes: `npm install -g @anthropic-ai/Codex`
2. Desde la raíz del proyecto, registra cada MCP con su token (sustituye el valor):

```powershell
Codex mcp add-json supabase '{\"command\":\"npx\",\"args\":[\"-y\",\"@supabase/mcp-server-supabase@latest\",\"--project-ref\",\"bbmglaatlyilxutzomxd\"],\"env\":{\"SUPABASE_ACCESS_TOKEN\":\"TU_PAT_DE_SUPABASE\"}}'

Codex mcp add-json linear '{\"command\":\"npx\",\"args\":[\"-y\",\"mcp-linear@latest\"],\"env\":{\"LINEAR_API_KEY\":\"TU_API_KEY_DE_LINEAR\"}}'
```

3. Verifica con `Codex mcp list` — deberías ver ambos como `✓ Connected`.
4. Reinicia Codex Desktop por completo (cierra ventana e icono de la bandeja del sistema) y abre sesión nueva.

`.Codex/settings.json` y `.Codex/settings.local.json` se mantienen como referencia de la estructura, pero la fuente de verdad funcional es el registro de la CLI.
...
- ✅ **CAM-52** — `POST /api/chat/buyer/message`: streaming Codex (Vercel AI SDK) + persistencia de mensajes
...
app/api/chat/buyer/
  ...
  message/route.ts   — POST: streaming Codex + tool use register_buyer_lead (crea BuyerLead en execute)
...
2. Servidor: paso 1 — Codex llama `register_buyer_lead` con los datos capturados
...
4. Paso 2: Codex genera texto cálido de confirmación ("Un agente te contactará en 24h")
...
**Modelo por defecto**: `Codex-haiku-4-5-20251001` (configurable vía `ANTHROPIC_MODEL` en `.env.local`).  
...
ANTHROPIC_MODEL=Codex-haiku-4-5-20251001  # opcional; este es el default
```
