...
> ⚠️ **Estado VIGENTE del programa I3 (coordinación de escritores):** I3A, I3B, I3C1A, I3C1B e I3C2
> están **fusionados y desplegados**. **I3C3** (compleción coordinada) está **preparado en rama/PR,
> sin fusionar ni desplegar ni validar** (sin migración): `completeDeliveryTx` adopta `withLockedRoots`
> (relee y valida checklist/firma **bajo el lock**, cierra Match/Buyer, Vehicle→VENDIDO + `soldAt`,
> Warranty + 2 follow-ups, todo atómico; **admite leads archivados** sin reactivar; **no reversible**).
> Además, la **edición de checklist** (`updateChecklistItemTx`) y la **firma** (`writeSignatureTx`)
> entran en el MISMO protocolo de root locks (`lib/delivery-precondition.ts`): relean bajo el lock y
> rechazan en estados terminales, **cerrando el TOCTOU checklist/firma↔compleción end-to-end** (sin
> ventana residual; carreras demostradas con PostgreSQL real). **I3D** (descarte) e **I3E**
> (tasación) siguen **pendientes**. Con I3C3 hay **8 callers productivos** de `withLockedRoots`
> (verificar contra código). `LEAD_ARCHIVED` **ya es
