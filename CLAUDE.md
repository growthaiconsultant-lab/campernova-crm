...
- 🚀 **LANZAMIENTO a producción (2026-06-03)** — ver **`docs/LAUNCH.md`** (fuente de verdad del go-live + lo pendiente + rollback):
  ...
  - 🔴 **Pendiente inmediato tras cutover** (no bloquea, web funciona): **`NEXT_PUBLIC_APP_URL` → `https://campersnova.com` + redeploy** (el sitemap aún lista URLs de `vercel.app`; los canonicals ya usan el dominio real); **Supabase Auth** (Site URL + `campersnova.com/auth/callback` para el login del equipo en el dominio); **`www.campersnova.com`** (aún apunta a WordPress, falta redirect al apex). Pasos exactos en `docs/LAUNCH.md`.
