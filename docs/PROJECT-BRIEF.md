...
- **Observación 24 h** (Sentry/Vercel) de A2 y **de A3** — **no** completada. Smoke técnico inmediato de
  A3 en prod + **smoke autenticado read-only (sesión ADMIN legítima): sin errores A3** (Sentry limpio de A3).
- **Incidencia independiente (no A3):** hidratación React (#422/#425) en `/compradores/[id]`
  (`CAMPERNOVA-CRM-G`), pre-existente al deploy A3, recuperable, 0 usuarios → **tarea separada** de baja
  prioridad; no se corrige junto a A3.
