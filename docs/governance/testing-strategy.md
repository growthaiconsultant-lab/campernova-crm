...
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------- |
| **Unitarios**                      | **721** tests, lógica pura, validaciones, reglas, helpers, servicios con deps inyectadas                               | reglas deterministas, casos límite, errores de dominio, regresiones rápidas | `pnpm test`                                                |
| **Integración PostgreSQL**         | **59** tests (10 ficheros), PostgreSQL 17 efímero; transacciones, CAS, constraints, concurrencia, operaciones críticas | garantías que dependen de la DB                                             | `pnpm test:integration` (prep: `test:integration:prepare`) |
| **Migration replay**               | 2 migraciones, baseline, catálogo **31/431/49/258/65/111**, parity, RLS, idempotencia                                  | reproducibilidad del esquema desde vacío                                    | job CI `migration-replay`                                  |
| **Supabase local**                 | **19** tests (2 ficheros); buckets, policies, RLS, Storage, guard anti-remoto                                          | Storage real                                                                | `pnpm test:supabase`                                       |
