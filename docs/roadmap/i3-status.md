...
| **I3C3**  | Compleción coordinada (Delivery→COMPLETADA, Vehicle→VENDIDO, `soldAt`, Match/Buyer, Warranty, follow-ups) |     —     |           —            |     —      |                          —                           | **no iniciado** (requiere auditoría read-only previa) |
...
- **I3C3, I3D, I3E: pendientes** (no iniciados).
- **Guardas de checklist/firma en estados terminales: pendientes** — a decidir durante la auditoría de
  I3C3 (no asignadas automáticamente a ninguna fase).
- **Validación autenticada del flujo de entrega en producción: pendiente** (`AUTHENTICATED DELIVERY
FLOW VALIDATION PENDING`).
- **Callers productivos de `withLockedRoots`: 5** (createOffer, updateOfferStatus, updateVehicle,
  createDelivery, transición/cancelación de Delivery) — verificar contra código al cambiar.
...
| ---------------------------------------- | --------- | --------------------------- | -------------------------- |
| Compleción coordinada (locks + carreras) | pendiente | I3C3                        | auditoría read-only previa |
| Guardas terminales checklist/firma       | pendiente | a decidir en auditoría I3C3 | ¿entra en I3C3?            |
